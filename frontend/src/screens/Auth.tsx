...
import { useAuth, getFriendlyErrorMessage } from "../context/AuthContext";
...
export default function Auth({ initial, go }: { initial: Mode; go: Go }) {
  ...
  if (mode === "login") {
    ...
    const handleGoogleLogin = async () => {
      setError(null);
      setLoading("Connecting with Google…");
      try {
    ...
    const handleDemoLogin = async () => {
      setError(null);
      setLoading("Starting demo session…");
      try {
        await loginAsDemo();
        go("home");
      } catch (err: any) {
      ...
      <AuthShell go={go}>
        ...
        <form className="mt-7 space-y-4" onSubmit={handleLoginSubmit}>
          ...
          <div className="flex justify-end">
            <button type="button" onClick={() => { setMode("forgot"); setError(null); }} className="text-sm font-medium text-emerald-500 hover:underline">
              Forgot password?
            </button>
          </div>
          {error && (
        ...
        <div className="my-5 flex items-center gap-3 text-xs text-medium-gray">
        ...
        <div className="space-y-2.5">
          ...
          <button
            ...
            disabled={!!loading}
            className="flex h-11 w-full items-center justify-center gap-2 rounded-lg border border-emerald-500/40 bg-emerald-50/80 text-sm font-semibold text-forest-700 transition-colors hover:bg-emerald-100 focus:outline-none focus:ring-2 focus:ring-emerald-500/40 disabled:opacity-60"
          >
            <Icon.Leaf size={16} className="text-emerald-600" />
            <span>{loading === "Starting demo session…" ? "Launching Demo…" : "1-Click Instant Demo"}</span>
          </button>
        </div>
  ...
  if (mode === "signup") {
    ...
    return (
      <AuthShell go={go}>
        ...
        <form className="mt-7 space-y-4" onSubmit={handleSignupSubmit}>
  ...
  if (mode === "forgot") {
    ...
    return (
      <AuthShell go={go}>
        <button onClick={() => { setMode("login"); setError(null); }} className="mb-6 inline-flex items-center gap-1.5 text-sm text-medium-gray hover:text-forest-700">
      ...
      <AuthShell go={go}>
        <div className="text-center">
          ...
          <div className="mt-7 space-y-3">
            <Button className="w-full" size="lg" onClick={() => { setMode("login"); setError(null); }}>Back to login</Button>
            <button
  ...
  return (
    <AuthShell go={go}>
      ...
      <div className="mt-7 space-y-3">
        <Button className="w-full" size="lg" icon="Check" onClick={() => go("home")}>
          Continue to Home
        </Button>
        <Button
          className="w-full"
          ...
          onClick={() => {
            setResending(true);
            setTimeout(() => setResending(false), 2500);
          }}
        >
          {loading === "Resending…" ? "Resending…" : "Resend Email"}
        </Button>
        <button onClick={() => { setMode("signup"); setError(null); }} className="block w-full text-center text-sm font-medium text-slate-gray hover:text-forest-700">
          Change email
        </button>
      </div>
    </AuthShell>
