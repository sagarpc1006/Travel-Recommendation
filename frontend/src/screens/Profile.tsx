...
export default function Profile({ go }: { go: Go }) {
  ...
  function flash(m: string) { setToast(m); window.setTimeout(() => setToast(null), 2200); }
    ...
    <AppShell active="profile" go={go}>
      <main className="mx-auto max-w-4xl px-6 py-8">
        ...
        <div className="flex flex-wrap items-center gap-4">
          <span className="grid h-16 w-16 place-items-center rounded-2xl bg-sage-200 text-xl font-semibold text-forest-700">AK</span>
          <div className="flex-1">
            <h1 className="text-2xl font-bold tracking-tight text-near-black">Aditi Kulkarni</h1>
            <div className="mt-0.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-slate-gray">
              <span className="flex items-center gap-1"><Icon.Profile size={14} /> aditi@example.com</span>
              <Badge icon="Verified" label="Verified account" tone={TONE.verified} />
            </div>
          </div>
          <Button variant="tertiary" icon="Sliders" onClick={() => flash("Edit profile coming soon.")}>Edit profile</Button>
        </div>
        ...
        <Section title="Travel preferences" icon="Sliders">
          <div className="grid gap-3 sm:grid-cols-2">
            <PrefPill label="Travel style" value="Nature & culture" />
            <PrefPill label="Budget" value="Moderate" />
            <PrefPill label="Convenience" value="Balanced" />
            <PrefPill label="Sustainability" value="High priority" />
          </div>
          <Button variant="tertiary" size="sm" className="mt-3" icon="Sliders" onClick={() => go("onboarding")}>Update preferences</Button>
        </Section>
        ...
        <Section title="Account" icon="Settings">
          <div className="divide-y divide-border rounded-xl border border-border">
            {[
              ...
              const I = Icon[ic as IconName];
              return (
                <button key={t} onClick={() => flash(`${t} settings coming soon.`)} className="flex w-full items-center gap-3 px-4 py-3 text-left hover:bg-soft-gray">
          ...
          <Button variant="tertiary" className="mt-4" icon="Close" onClick={() => go("landing")}>Log out</Button>
        </Section>
      </main>
